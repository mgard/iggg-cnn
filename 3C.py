...
w_hidden1 = tf.Variable(tf.truncated_normal([3072, 768], stddev=0.3))
w_hidden2 = tf.Variable(tf.truncated_normal([768, 384], stddev=0.3))
w_hidden3 = tf.Variable(tf.truncated_normal([384, 128], stddev=0.3))
w_output = tf.Variable(tf.truncated_normal([128, 10], stddev=0.3))
b_hidden1 = tf.Variable(tf.zeros([768]))
b_hidden2 = tf.Variable(tf.zeros([384]))
b_hidden3 = tf.Variable(tf.zeros([128]))
...
pred = tf.nn.sigmoid(tf.matmul(pred, w_hidden3) + b_hidden3)
...
writer = tf.train.SummaryWriter("/tmp/tflogs_3C", sess.graph_def, flush_secs=10)
