...
pred = tf.nn.relu(tf.nn.conv2d(pred, w_c2, strides=[1, 1, 1, 1], padding='SAME'))
pred = tf.nn.max_pool(pred, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

pred = tf.nn.relu(tf.nn.conv2d(pred, w_c3, strides=[1, 1, 1, 1], padding='SAME'))
pred = tf.nn.max_pool(pred, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
...
writer = tf.train.SummaryWriter("/tmp/tflogs_4B", sess.graph_def, flush_secs=10)
